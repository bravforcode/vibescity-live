...
const uniqueEventKey = (event) => {
	...
	if (eventName && range?.start) {
		const dayKey = range.start.toISOString().slice(0, 10);
		const venueId = String(event?.venueId || event?.venue_id || "").trim();
		const locationKey =
			venueId ||
			(() => {
				...
				return normalizeText(
					event?.location || event?.venue || event?.province || "",
				);
			})() ||
			"unknown";
		return `event-slot:${eventName}:${dayKey}:${locationKey}`;
	}
