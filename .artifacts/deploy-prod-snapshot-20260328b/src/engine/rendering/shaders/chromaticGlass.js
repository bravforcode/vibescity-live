...
export const CHROMA_BLUR_H_FRAG = /* glsl */ `
  ...
  // 13-tap Gaussian kernel weights (sigma ≈ 2.5) — GLSL ES 1.00 compatible
  float getW(int i) {
    if (i == -3 || i == 3) return 0.0044;
    if (i == -2 || i == 2) return 0.0540;
    if (i == -1 || i == 1) return 0.2420;
    return 0.3990;
  }
  ...
  void main() {
    ...
    vec4 color = vec4(0.0);
    for (int i = -3; i <= 3; i++) {
      color += texture2D(u_texture, vUv + float(i) * px) * getW(i);
    }
    gl_FragColor = color;
  }
`;
...
export const CHROMA_BLUR_V_FRAG = /* glsl */ `
  ...
  // 13-tap vertical Gaussian — GLSL ES 1.00 compatible
  float getW(int i) {
    if (i == -3 || i == 3) return 0.0044;
    if (i == -2 || i == 2) return 0.0540;
    if (i == -1 || i == 1) return 0.2420;
  ...
  void main() {
    ...
    for (int i = -3; i <= 3; i++) {
      blurred += texture2D(u_texture, vUv + float(i) * px) * getW(i);
    }
