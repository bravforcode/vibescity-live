	...
	return page
		.waitForFunction(
			(selector) => {
				...
				return (
					shell?.getAttribute("data-map-ready") === "true" &&
					shell?.getAttribute("data-map-content-ready") === "true"
				);
			},
			MAP_SHELL_SELECTOR,
			{ timeout: timeoutMs },
	...
	if (!isReady) {
		const shellMeta = await mapShell
			.evaluate((el) => ({
				...
				contentReady: el.getAttribute("data-map-content-ready"),
				styleMode: el.getAttribute("data-map-style-mode"),
		...
		const reasonParts = [
			...
			shellMeta
				? `Map shell attrs: ready=${shellMeta.ready}, contentReady=${shellMeta.contentReady}, styleMode=${shellMeta.styleMode}, init=${shellMeta.initRequested}, tokenInvalid=${shellMeta.tokenInvalid}.`
				: null,
		].filter(Boolean);
