...
const PRERENDER_ENABLED = (() => {
	const raw = String(process.env.PRERENDER_ENABLED ?? "true")
		.trim()
		.toLowerCase();
...
const PRERENDER_REQUIRED = (() => {
	const raw = String(process.env.PRERENDER_REQUIRED ?? "")
		.trim()
		.toLowerCase();
...
if (!PRERENDER_ENABLED) {
	process.stdout.write(
		"Prerender disabled (PRERENDER_ENABLED=false). Skipping.\n",
	);
	process.exit(0);
}
...
const venueToJsonLd = (v, canonicalUrl, locale) => {
	const images = Array.isArray(v.image_urls)
		? v.image_urls
		: [v.Image_URL1, v.Image_URL2, v.Image_URL3].filter(
...
const injectCategorySeo = (
	...
	const description = clampText(buildCategoryDescription(name), 155);
...
const fetchVenues = async () => {
	...
	const isMissingShortCodeColumn = (error) =>
		isMissingColumn(error, "short_code");
	...
	const tryFetchPaged = async (tableName, fields) => {
		...
		for (;;) {
			...
			out.push(...rows);
			if (!rows.length || rows.length < PAGE_SIZE || out.length >= MAX_VENUES)
				break;
			offset += PAGE_SIZE;
		}
	...
	// Prefer venues_public view when available.
	{
		...
		if (!attempt.ok && isMissingShortCodeColumn(attempt.error)) {
			const retry = await tryFetchPaged(
				"venues_public",
				fieldsPublicWithSlugOnly,
			);
...
const main = async () => {
	...
	for (const v of venues) {
		...
		for (const locale of LOCALES) {
			...
			{
				const legacyPath = buildLocalePath(
					`/venue/${encodeURIComponent(id)}`,
					locale,
				);
