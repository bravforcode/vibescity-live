...
const overpassTimeoutSec = Math.max(
	15,
	Number(args.overpassTimeout || args.overpass_timeout || 90),
);
const includeNameless = String(args.includeNameless || args.include_nameless || "0") === "1";
const strategy = String(args.strategy || "hybrid").toLowerCase(); // hybrid | around
const allowNearestFallback =
	String(args.allowNearestFallback || args.allow_nearest_fallback || "1") ===
	"1";
const nearestMaxDistanceKm = Math.max(
	1,
	Number(args.nearestMaxKm || args.nearest_max_km || 80),
);
...
if (!["hybrid", "around"].includes(strategy)) {
...
function pickBestCandidate(seed, candidates, usedKeys) {
	...
	if (!best) return null;
	return { candidate: best, distanceKm: bestDistanceKm, matchStrategy: "polygon" };
}

function pickNearestCandidate(seed, candidates, usedKeys, maxDistanceKm) {
	let best = null;
	let bestDistanceKm = Infinity;
	const seedPoint = turf.point([Number(seed.longitude), Number(seed.latitude)]);
	for (const candidate of candidates) {
	...
	if (!best) return null;
	if (bestDistanceKm > maxDistanceKm) return null;
	return {
		candidate: best,
		distanceKm: bestDistanceKm,
		matchStrategy: "nearest_fallback",
	};
}
...
function buildReplacementRow(seed, match, nowIso) {
	...
	return {
		...
		metadata: {
			...
			match_strategy: match.matchStrategy || "polygon",
	...
	for (const radius of fallbackRadii) {
		...
		if (allowNearestFallback) {
			const nearest = pickNearestCandidate(
				...
				candidates,
				usedKeys,
				nearestMaxDistanceKm,
			);
			if (nearest) {
				if (
					!nearestFallbackMatch ||
					nearest.distanceKm < nearestFallbackMatch.distanceKm
				) {
					nearestFallbackMatch = nearest;
				}
			}
		}
	}
	return nearestFallbackMatch;
}
...
async function buildPlan() {
	...
	let fetchMeta = [];
	if (strategy === "hybrid") {
		const poolResult = await fetchProvincePools(districtFirstSeeds, expected);
		for (const [provinceCode, candidates] of poolResult.byProvince.entries()) {
			byProvince.set(provinceCode, candidates);
		}
		fetchMeta = poolResult.fetchMeta;
	}
	...
	for (let index = 0; index < districtFirstSeeds.length; index += 1) {
		...
		if (strategy === "hybrid") {
			...
			match = pickBestCandidate(seed, provincePool, usedKeys);
			if (!match && allowNearestFallback) {
				match = pickNearestCandidate(
					seed,
					provincePool,
					usedKeys,
					nearestMaxDistanceKm,
	...
	return {
		...
		config: {
			...
			strategy,
			allow_nearest_fallback: allowNearestFallback,
			nearest_max_distance_km: nearestMaxDistanceKm,
