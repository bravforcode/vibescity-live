...
import { isAsyncChunkLoadError } from "./runtimeErrorGuards";
...
export const defineResilientAsync = (loader, opts = {}) =>
	defineAsyncComponent({
		loader,
		errorComponent: opts.errorComponent ?? AsyncFallback,
		timeout: opts.timeout ?? 15000,
		delay: opts.delay ?? 200,
		loadingComponent: opts.loadingComponent ?? undefined,
		onError(error, retry, fail, attempts) {
			const maxChunkRetries = opts.chunkRetryCount ?? 1;
			if (isAsyncChunkLoadError(error) && attempts <= maxChunkRetries) {
				globalThis.setTimeout(() => retry(), opts.retryDelayMs ?? 200);
				return;
			}
			fail();
		},
