...
function isMissingPiiAuditTableError(error) {
	const message = String(error?.message || error || "");
	const code = String(error?.code || "");
	return (
		code === "PGRST205" ||
		(message.includes("pii_audit_access_log") &&
...
function buildActorStats(rows) {
	...
	for (const row of rows) {
		...
		if (
			createdAt &&
			(!current.last_seen_at || createdAt > current.last_seen_at)
		) {
...
async function main() {
	...
	const totalThreshold = toInt(
		process.env.PII_ACCESS_ALERT_TOTAL_THRESHOLD,
		50,
	);
	...
	const exportCount = await fetchCount(
		adminClient,
		fromIso,
		toIsoValue,
		"export",
...
main().catch(async (error) => {
	if (isMissingPiiAuditTableError(error)) {
		const outputPath =
			process.env.PII_ACCESS_ALERT_OUTPUT_PATH ||
			"reports/ci/pii-access-anomaly-log.json";
		const payload = {
			...
			status: "skipped",
			reason:
				"pii_audit_access_log table is not provisioned in this environment",
		};
		await mkdir(getParentDir(outputPath), { recursive: true });
		await writeFile(outputPath, JSON.stringify(payload, null, 2));
		await appendSummary([
			"## PII Access Anomaly Check",
			"- Status: SKIP",
			"- Reason: pii_audit_access_log table is not provisioned in this environment",
		]);
		if (process.env.GITHUB_OUTPUT) {
			await appendFile(process.env.GITHUB_OUTPUT, "alert=false\nalert_text=\n");
		}
		return;
	}
