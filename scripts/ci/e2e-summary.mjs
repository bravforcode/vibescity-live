...
function parseConsoleEntries(inputPath) {
	...
	for (const line of lines) {
		try {
			...
			// Ignore malformed console lines in summary mode.
		}
	}
	return entries;
}
...
function parseJUnit(parsed) {
	...
	for (const suite of suites) {
		...
		for (const testCase of cases) {
			...
			const hasFailure =
				toArray(testCase.failure).length > 0 ||
				toArray(testCase.error).length > 0;
...
async function main() {
	...
	const consolePath = arg("--console", "test-results/ui-console-events.ndjson");
	...
	try {
		...
		const consoleEntries = parseConsoleEntries(consolePath);
		const lines = [
			header,
			`- Total: ${total}`,
			`- Passed: ${passed}`,
