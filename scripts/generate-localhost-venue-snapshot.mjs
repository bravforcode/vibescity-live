...
const supabase = canUseSupabaseSnapshot
	...
	: null;
const shouldReuseExistingArtifacts =
	process.env.VIBECITY_REUSE_EXISTING_LOCALHOST_SNAPSHOT === "1";
...
const toCompleteVenueRow = (row, mediaPayload) => {
	...
	if (!coords || !mediaPayload?.coverage?.has_media) return null;
	...
	return {
		...
		has_real_image: mediaPayload.coverage.has_images,
		has_real_video: mediaPayload.coverage.has_videos,
...
const isExistingMediaIndexAuthoritative = (payload) => {
	...
	return rows.every((row) => {
		...
		return (
			(Number(counts.images) > 0 || Number(counts.videos) > 0) &&
...
const isExistingSnapshotAuthoritative = (payload) => {
	...
	return rows.every((row) => {
		const coverage = row?.coverage || {};
		return coverage.has_media === true;
	});
};
...
const main = async () => {
	if (
		shouldReuseExistingArtifacts &&
		(await hasExistingAuthoritativeArtifacts())
	) {
		process.stdout.write(
			`Reusing existing localhost snapshot artifacts -> ${OUTPUT_SNAPSHOT_PATH}\n`,
		);
		process.stdout.write(
			`Reusing existing real media index -> ${OUTPUT_MEDIA_INDEX_PATH}\n`,
		);
		return;
	}

	let completeRows = [];
	let sourcePath = "supabase:venues+venue_photos_any_real_media";
	let sourceRowCount = 0;
	let selectionStrategy =
		"thailand_grid_round_robin_any_real_media_plus_major_city_coverage";

	try {
		process.stdout.write("Fetching venue rows for localhost snapshot...\n");
		const venueRows = await fetchVenueRows();
		process.stdout.write(
			`Fetched ${venueRows.length} venue rows, building media coverage map...\n`,
		);
		const photoMap = await buildPhotoMap(
			venueRows.map((row) => normalizeText(row.id)).filter(Boolean),
		);
		process.stdout.write("Building localhost snapshot payloads...\n");
		...
		if (!completeRows.length) {
			throw new Error("SUPABASE_ANY_MEDIA_ROWS_EMPTY");
		}
	} catch (error) {
