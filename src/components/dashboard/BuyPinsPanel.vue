<template>
  ...
  >
    ...
    <div
      class="flex flex-row overflow-x-auto snap-x snap-mandatory gap-4 pb-4 md:grid md:grid-cols-2 lg:grid-cols-4 md:overflow-visible md:pb-0 md:gap-6"
      style="scrollbar-width: none; -webkit-overflow-scrolling: touch;"
    >
      <div
        v-for="(pkg, idx) in filteredPackages"
        :key="pkg.sku"
        class="@container/card bg-white/5 rounded-2xl p-1 border border-white/10 hover:border-white/30 transition group relative flex-shrink-0 w-[17.5rem] md:w-auto md:min-w-0 snap-center flex flex-col"
...
const normalizeTransferProfileType = (
	value: unknown,
): TransferProfile["type"] =>
	String(value || "") === "international_wire"
		? "international_wire"
		: "thai_bank";
...
const parseTransferProfiles = (): TransferProfile[] => {
	...
	try {
		...
		return parsed
			.map((profile): TransferProfile | null => {
				const id = String(profile?.id || "");
				const label = String(profile?.label || "");
				if (!id || !label) return null;
				return {
					id,
					type: normalizeTransferProfileType(profile?.type),
					label,
			...
			.filter((profile): profile is TransferProfile => Boolean(profile));
	} catch {
		return DEFAULT_TRANSFER_PROFILES;
	}
};
