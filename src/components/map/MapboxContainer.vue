...
watch(
  () => props.userLocation,
  (newLoc, oldLoc) => {
    if (newLoc && Array.isArray(newLoc) && newLoc.length >= 2) {
      const newCenter = [newLoc[1], newLoc[0]]; // [lng, lat]

      // If map is not initialized yet, update the initial center ref
      if (!mapInitRequested.value || !isMapReady.value) {
        center.value = newCenter;
      }
      // If it's the first time we get a valid physical location and the map is ready, pan to it
      else if (!oldLoc && map.value) {
        try {
          if (typeof map.value.flyTo === "function") {
            map.value.flyTo({ center: newCenter, zoom: 16.5, essential: true });
          }
        } catch (e) {
          console.warn("Mapbox flyTo failed on location update:", e);
        }
      }
    }
  },
  ...
  if (!map.value.getLayer(SELECTED_PIN_LAYER_ID)) {
    map.value.addLayer({
      ...
      layout: {
        ...
        "icon-size": ["case", ["==", ["get", "pin_type"], "giant"], 0.4, 0.3],
        "icon-allow-overlap": true,
...
sentient = useSentientMap(
  ...
  emit,
  {
    ...
    // Optional: remote config override
    // config: { enabled: true, killSwitch: false, ... }
  },
);
