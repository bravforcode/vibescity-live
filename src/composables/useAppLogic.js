...
export function useAppLogic() {
    ...
    const {
        isMobileView, isLandscape, bottomUiHeight,
        mobileCardScrollRef, showSidebar, showMallDrawer,
        errorMessage, showConfetti, activePopup
    } = uiLogic;
    ...
    const {
        activeEvents, buildingsData, realTimeEvents,
        updateEventsData, timedEvents
    } = eventLogic;
    ...
    const { handleHorizontalScroll, scrollToCard, onScrollStart, onScrollEnd } = scrollSync;
    ...
    const globalSearchResults = computed(() => {
		...
		const emojiMap = {
			"☕": "cafe", "🍽️": "restaurant", "🍜": "food", "🍺": "bar",
			"🍷": "wine", "💃": "club", "🎵": "live music", "🎨": "art",
			"🛍️": "fashion", "🏢": "mall",
		};
		...
		if (userLocation.value) {
			...
			return matches
				.map((s) => ({
					...s,
					distance: s.lat && s.lng ? calculateDistance(uLat, uLng, s.lat, s.lng) : Infinity,
				}))
    ...
    const applyShopSelection = (shopId, autoImmersive = false) => {
		...
		const shop = shopStore.visibleShops.find((s) => Number(s.id) == Number(shopId));
		if (shop) {
    ...
    const handleCardClick = (shop) => {
        ...
        // Video sync logic
		const videoEl = document.querySelector(`div[data-shop-id="${shop.id}"] video`);
		if (videoEl) {
    ...
    const updateMetadata = () => {
        ...
        const currentShop = shopStore.visibleShops.find(s => s.id == activeShopId.value);
    ...
    watch(() => [activeShopId.value, activeCategories.value], updateMetadata, { immediate: true });
    ...
    watch(activeCategories, (newCats) => {
		...
		if (["nightlife", "bar", "club", "wine"].some((k) => cat.includes(k))) setZone("nightlife");
		else if (["nature", "park", "camp"].some((k) => cat.includes(k))) setZone("nature");
		else if (["temple", "culture"].some((k) => cat.includes(k))) setZone("temple");
    ...
    const initApp = async () => {
        ...
        try {
            ...
            await Promise.all([
				...
				updateEventsData(),
                fetch("/data/buildings.json").then(r => r.json()).then(d => buildingsData.value = d).catch(()=>{}),
                fetch("/data/events.json").then(r => r.json()).then(d => timedEvents.value = d).catch(()=>{})
            ]);

            // Geolocation
            if(navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    (pos) => shopStore.setUserLocation([pos.coords.latitude, pos.coords.longitude]),
    ...
    return {
        ...
        handleEnterGiantView: (b) => mapLogic.handleEnterGiantView(b, selectFeedback),
        ...
        toggleOwnerDashboard: () => isOwnerDashboardOpen.value = !isOwnerDashboardOpen.value,
        ...
        handleSearchBlur: () => setTimeout(() => uiLogic.showSearchResults.value = false, 200),
        ...
        mallShops: computed(() => {
            if(!activeBuilding.value) return [];
             const targetKey = String(activeBuilding.value.key || "").trim().toLowerCase();
             return shopStore.rawShops.filter(s => String(s.Building||"").trim().toLowerCase() === targetKey);
        }),
        activeFloor,
