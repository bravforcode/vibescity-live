...
const normalizeEvent = (rawEvent) => {
  ...
  return {
    ...
    shortName: rawEvent.shortName || rawEvent.name?.split(' ').slice(0, 3).join(' '),
    ...
    startTime: rawEvent.startTime || rawEvent.start?.local || new Date().toISOString(),
    endTime: rawEvent.endTime || rawEvent.end?.local || new Date(Date.now() + 86400000).toISOString(),
