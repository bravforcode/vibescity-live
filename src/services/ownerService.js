...
import { apiFetch, parseApiJson } from "./apiClient";
...
const parseApiPayload = async (response, fallbackMessage) => {
	...
	return withSource(payload?.data, API_SOURCE);
};
...
export const ownerService = {
	...
	async getVenues(providedVisitorId = "", rawOptions = {}) {
		const options =
		...
		if (cursorCreatedAt)
			query.set("cursor_created_at", String(cursorCreatedAt));
		if (cursorId) query.set("cursor_id", String(cursorId));

		return callOwnerApiOrFallback({
			path: `/owner/venues?${query.toString()}`,
