...
export const useShopStore = defineStore('shop', () => {
  ...
  const collectedCoins = ref(new Set(JSON.parse(localStorage.getItem(COINS_STORAGE_KEY) || "[]")));
  const totalCoins = ref(parseInt(localStorage.getItem(COINS_TOTAL_KEY) || "0"));

  // ✅ Review System State
  const REVIEWS_STORAGE_KEY = "vibecity_user_reviews";
  const reviews = ref(JSON.parse(localStorage.getItem(REVIEWS_STORAGE_KEY) || "{}"));
  ...
  const levelProgress = computed(() => {
    ...
    const prevXP = userLevel.value === 1 ? 0 : 
                   userLevel.value === 2 ? 100 :
                   userLevel.value === 3 ? 300 :
                   userLevel.value === 4 ? 600 : 1000;
  ...
  const nearbyShops = (userLoc) => {
    ...
    const isDefaultView = activeCategories.value.length === 0 && activeStatus.value === 'ALL';
    if (isDefaultView) {
      const liveShops = candidates.filter((s) => s.status === 'LIVE' || s.Status === 'LIVE');
      const normalShops = candidates.filter((s) => s.status !== 'LIVE' && s.Status !== 'LIVE');
  ...
  const saveCoins = () => {
    localStorage.setItem(COINS_STORAGE_KEY, JSON.stringify([...collectedCoins.value]));
