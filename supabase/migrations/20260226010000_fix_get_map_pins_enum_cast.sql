-- -----------------------------------------------------------------------------
-- Fix: restore ::text cast on status/pin_type in get_map_pins
-- Regression: 20260225113000 removed the ::text cast introduced by the
-- 20260224220000 hotfix, causing 500 errors when venue_status is an enum type.
-- PostgreSQL cannot implicitly cast string literals to enum in NOT IN context.
-- Solution: re-apply ::text casts; use single-branch with zoom-sensitive LIMIT.
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  WHERE v.location IS NOT NULL
    ...
    -- ::text cast is required when status column is a venue_status enum type
