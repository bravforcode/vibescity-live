-- -----------------------------------------------------------------------------
-- Fix: restore IF/ELSE zoom branches for optimal planner behaviour + ANALYZE
-- Problem: single-branch function prevents per-zoom query plan optimization.
--          New GIST index needs ANALYZE before planner will prefer it.
-- Status cast: status::text works in function body (no IMMUTABLE restriction).
-- -----------------------------------------------------------------------------
SET statement_timeout = 0;
SET lock_timeout = '10s';

-- Refresh planner stats so the GIST index is used
