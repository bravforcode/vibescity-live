-- =============================================================================
-- Migration: Tune Province Aggregate RPC v2 Timeout and Filter Matching
-- Date: 2026-03-09
-- Purpose:
--   1. Prevent PostgREST statement timeout on heavy province filter requests
--   2. Normalize category matching to avoid case/spacing mismatches
-- =============================================================================
...
AS $$
  WITH normalized_categories AS (
    SELECT ARRAY(
      SELECT DISTINCT lower(trim(category_value))
  ...
  source_venues AS (
    ...
    WHERE v.deleted_at IS NULL
      ...
      AND trim(v.province) <> ''
      AND (
        COALESCE(array_length(nc.values, 1), 0) = 0
        OR lower(trim(COALESCE(v.category, ''))) = ANY(nc.values)
      )
      AND (
