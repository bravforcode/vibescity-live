...
test.describe("VibeCity – Smoke Tests", { tag: "@smoke" }, () => {
	...
	test("merchant route renders portal shell without blank screen", async ({
		page,
	}) => {
	...
	test("partner route stays on-page for non-member and shows gate shell", async ({
		...
		await page.route("**/rest/v1/feature_flags_public*", async (route) => {
			await route.fulfill({
				...
				body: JSON.stringify([
					...
					{
						key: "ff_map_shader_safe_mode_v1",
						enabled: true,
						rollout_percent: 100,
						kill_switch: false,
						config: {},
		...
		await page.route("**/partner/status**", async (route) => {
		...
		await page.route("**/partner/dashboard**", async (route) => {
			await new Promise((resolve) => setTimeout(resolve, 250));
			await route.fulfill({
				status: 200,
				contentType: "application/json",
	...
	test("deeplink /venue/:id opens venue detail sheet", async ({ page }) => {
		...
		const mediaVisible = await page
			.getByTestId("vibe-modal-media")
			.first()
			.isVisible({ timeout: 5_000 })
			.catch(() => false);
		const fallbackVisible = await page
			.getByTestId("vibe-modal-fallback")
			.first()
			.isVisible({ timeout: 5_000 })
			.catch(() => false);
		expect(
			mediaVisible || fallbackVisible,
			"Venue modal should render media or the branded fallback poster",
		).toBeTruthy();
	});
