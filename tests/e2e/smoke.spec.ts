...
function locators(page: any) {
	return {
		...
		bottomFeed: page
			...
			.or(page.getByRole("heading", { name: /กำลังฮิตขณะนี้|Trending/i })),

		carousel: page
			.getByTestId("vibe-carousel")
			.or(page.locator("[data-testid='vibe-carousel']"))
			.or(page.getByRole("button", { name: "shop.details" }).first()),
...
test.describe("VibeCity – Smoke Tests", { tag: "@smoke" }, () => {
	...
	test.describe("WebGL-dependent tests @map-required", () => {
		test.beforeEach(async ({ page }) => {
			...
			enforceMapConditionOrSkip(
				mapReady,
				"Map did not render a usable surface.",
			);
		});
		...
		test("shop carousel loads with cards", async ({ page }) => {
			...
			const detailsButtons = page.getByRole("button", { name: "shop.details" });
			const detailsVisible = await detailsButtons
				.first()
				.isVisible({ timeout: 15_000 })
				.catch(() => false);
			if (detailsVisible) {
				await expect(detailsButtons.first()).toBeVisible({ timeout: 15_000 });
				const cardCount = await detailsButtons.count();
		...
		test("clicking a card opens modal", async ({ page }) => {
			...
			const detailsVisible = await detailsButton
				...
				.catch(() => false);
			const cardVisible =
				detailsVisible || (await shopCard.isVisible().catch(() => false));
